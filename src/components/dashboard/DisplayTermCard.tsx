...
const DisplayTermCard: React.FC<DisplayTermCardProps> = ({ term, isShowingGrades, typeRearrange }) => {
  ...
  return (
    <Link to={typeRearrange ? '' : `/home/${term.term_name}`} className="">
        ...
        {!isShowingGrades && typeRearrange &&
            <div ref={setNodeRef} style={style}
                ...
                <div {...attributes}
                    ...
                    <h1 className='md:text-3xl text-xl'>{term.term_name.split(' ')[0]}</h1>
                    {term.term_name.split(' ').length > 1 && <h1 className='md:text-4xl text-3xl font-medium'>‘{term.term_name.split(' ')[1].slice(-2)}</h1>}
                </div>
            </div>}
